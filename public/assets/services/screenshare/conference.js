...
var conference = function (config) {
    ...
    function defaultSocketResponse(response) {
        if (response.userToken == self.userToken) {console.log("1");  return;}
        if (isGetNewRoom && response.roomToken && response.broadcaster){console.log("2"); config.onRoomFound(response);} 
        if (response.newParticipant) {console.log("3"); onNewParticipant(response.newParticipant);}
        ...
        if (response.isScreensharing == 0){
            $('#rooms-list').html('');
            $('#videos-container').empty();
            $('#remoteScreenshare').attr('src', '');
            $('#remoteScreenshare').hide();
          ...
          if (!isExecuted){
           ...
           $("#socketIOScript").attr('src', '');
           $("#socketIOScript").attr('id', '');
           $("#ssScript").attr('src', '');
           $("#ssScript").attr('id', '');
        ...
        //    setTimeout(() => {
            ...
            // ---- this one ---
             ...
             script2.src= './assets/services/screenshare/socket.io.js';
             script2.setAttribute("id", "socketIOScript");
             head.append(script2);
           
             // ---- and this one ---
             var script5= document.createElement('script');
             script5.src= './assets/services/screenshare/screenshare.js';
             script5.setAttribute("id", "ssScript");
             head.append(script5);
         
            // }, 300);
            // $("#videoModal").load(location.href);
    ...
    function openSubSocket(_config) {
        console.log("openSubSocket1");
        console.log(_config.channel);
        if (!_config.channel) return;
        console.log('openSubSocket2');
        var socketConfig = {
        ...
        var socket = config.openSocket(socketConfig),
            ...
            gotstream,
            htmlElement = document.getElementById('remoteScreenshare'),//document.createElement('video'),
            inner = {},
            peer;
    ...
    function startBroadcasting() {
        defaultSocket && defaultSocket.send({
            ...
            isScreensharing: localStorage.getItem('screenSharing')
        });
        if (localStorage.getItem('screenSharing') == 1) setTimeout(startBroadcasting, 3000);
    ...
    return {
        ...
        joinRoom: function (_config) {
            ...
            if (!defaultSocket || defaultSocket.length == 0 || defaultSocket == {}){
                console.log("conference: IF");
                openDefaultSocket();
            }
            else{
                console.log("conference: ELSE");
                console.log(self.userToken);    console.log(_config.joinUser);
                defaultSocket.send({
                    participant: true,
                    userToken: self.userToken,
